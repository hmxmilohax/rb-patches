...
#rb2, tbrb, gdrb - "-v 5"
#rb3, dc, blitz - "-v 6"
ninja.variable("ark_version", "-v 4")
...
if args.platform in ["ps3", "xbox", "ps2"] and patchcreator == False:
    ninja.rule(
        "ark",
        #we add -f its its rb1 because its special
        f"$arkhelper dir2ark -n {hdr_name} $ark_version $ark_encrypt -f -s 4073741823 --logLevel error {ark_dir} {out_dir}",
