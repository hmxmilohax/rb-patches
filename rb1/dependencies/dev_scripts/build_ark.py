...
def build_patch_ark(xbox: bool, rpcs3_directory: str = None, rpcs3_mode: bool = False):
    ...
    try:
        if platform == "win32":
            cmd_build = f"dependencies\windows\\arkhelper.exe dir2ark _ark {build_location} -n {patch_hdr_version} -e -v 4 -f".split()
        elif platform == "darwin":
            cmd_build = f"dependencies/macos/arkhelper dir2ark _ark {build_location} -n {patch_hdr_version} -e -v 4 -f".split()
        else:
            cmd_build = f"dependencies/linux/arkhelper dir2ark _ark {build_location} -n {patch_hdr_version} -e -v 4 -f".split()
    ...
    if not failed:
        print("ARK built successfully!")
        return True
    else:
        print("Error building ARK!")
        return False